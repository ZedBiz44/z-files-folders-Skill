...
The skill owns business placement and naming decisions. Existing Drive tools own API execution, authentication, boundary enforcement, and private receipts. Existing knowledge-routing and publishing skills own knowledge-page placement and Notion writing. This avoids a second Drive implementation or static folder map.

Use the live folder map for destinations and the live guide for procedure. A companion's generic spaced filename example does not override the specifically assigned guide. Companion account/boundary requirements remain intact. Source retention overrides a generic direct-move/delete recipe during migration.

## Initial Source Review

Both requested pages were fetched in full on 2026-09-09. The map includes an inline Z-Knowledge-Key-Folders database. Its prose contains disagreements: ZEPP-Purchased-Products versus ZETP-Tools-Products; multiple ZZVA full names; and Z-Core Ventures versus Z-Core-Ventures. These are source ambiguities, not permission to rewrite the pages. Inspect relevant records and live destinations; hold unresolved items while clear work continues.

The repository was empty at initial inspection. Existing overlapping skills checked: z-drive-gog and z-knowledge-routing. The developer skill was read locally and its source repository fetched for currency.
