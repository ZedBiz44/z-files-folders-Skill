...
- Use the executing agent's approved connection. For OpenClaw Drive work, load `z-drive-gog`, inspect its private runtime registry, pass the account explicitly, and verify current GOG command help/schema. Do not substitute another person's connection to claim this agent works.
- Keep account, root/Shared Drive IDs, receipt path, and credentials in private runtime configuration, not this package. If the current Drive skill only supports a client boundary and the assignment needs a company boundary, report the configuration gap. Do not invent a client code or bypass it.
